...
class Month{
    ...
    setStartingDate=(date)=>{
         this.startingDay=  `${date}/${this.#monthKey}/2024`;
    }
    setEndingDate=(date)=>{
        return `${date}/${this.#monthKey}/2024`
    }
    setBudget=(amount)=>{
        let budget =amount;
        return budget; 
...
// Implementing Algorithm Shnobol;
    ...
    year.monthDays.forEach((day)=>{
        ...
        currentDayKeepUpArr.forEach((expenseKey)=>{
            //search for a similar Expense account object with the same account Name
            // let picNum=1;
            for(let i=0;i<accountRecords.length;i++){
                if(expenseKey.toLocaleLowerCase()===accountRecords[i].accountName.toLocaleLowerCase()){
                    //  console.log(accountRecords[i].accountId="8");
                    // accountRecords[i].encurredDates.push(temp.getDate());
                    let obj = {};
                ...
                // accountRecords[i].picNum=picNum;
                // picNum++;
                    // console.log(accountRecords[i].encurredDates);
                }
            }
        })
    })
    ...
    console.log(accountRecords[5]);
    console.log(accountRecords);

    // calculatinfg mean for the transportCost
...
for(let i=0;i<accountRecords.length;i++){
    if(accountRecords[i].accountName===item){
